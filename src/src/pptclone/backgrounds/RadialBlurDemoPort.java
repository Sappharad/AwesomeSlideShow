/**
 * RadialBlurDemoPort.java
 * Original author: Pepijn Van Eeckhoudt
 * Ported to PPTClone by Paul Kratt
 * 
 * An OpenGL demo of Radial blur
 * 
 * Note: All base code are belong to Pepijn Van Eeckhoudt.
 * Modifications made by Paul Kratt:
 * - Render resolution based on glPanel.
 * - Camera changes replaced with glTranslate
 * - Creation code moved into draw
 * - Lighting settings reset to defaults after completion
 * - Probably a few minor things I've forgotten
 */
...
public class RadialBlurDemoPort implements pptclone.SlideBackground{
    private boolean firstframe; //Is this the first frame of animation?
    ...
    /** Update the angle of the toroid **/
    ...
    /** Reduce this normal to unit vector **/
    ...
    private void processHelix(GL gl, GLU glu) {  // Draws A Helix
        ...
        gl.glTranslatef(0, 0, -100);  // Translate 50 Units Into The Screen
    ...
    private void drawBlur(GL gl, int times, float inc)  // Draw The Blurred Image
    {
        ...
        for (int num = 0; num < times; num++)  // Number Of Times To Render Blur
        {
            ...
            gl.glVertex2f(0, glPanel.screenheight);  // Second Vertex  (   0, 480 )

            // Texture Coordinate  ( 1, 0 )
            gl.glTexCoord2f(1 - spost, 0 + spost);  
            gl.glVertex2f(glPanel.screenwidth, glPanel.screenheight);  // Third Vertex    ( 640, 480 )

            // Texture Coordinate  ( 1, 1 )
            gl.glTexCoord2f(1 - spost, 1 - spost);  
            gl.glVertex2f(glPanel.screenwidth, 0);  // Fourth Vertex  ( 640,   0 )
